...
class SubjectSettingsDeleteStaticTextCallbackTest {
    ...
    SubjectSettingsDeleteStaticTextCallback callback;

    @BeforeEach
    void setUp() {
        callback = new SubjectSettingsDeleteStaticTextCallback(sender, autoHomeworkClient, executorClient);
    }

    @Test
    void supports_returnsTrue_forDelStaticPrefix() {
        assertThat(callback.supports(SubjectSettingsCallbackData.DEL_STATIC + "1")).isTrue();
    }
    ...
    void handle_inaccessibleMessage_answersWithError() {
        Update update = mockCallbackUpdate(42L, 0L, 0, false, SubjectSettingsCallbackData.DEL_STATIC + "1");
    ...
    void handle_noExistingPrompt_answersDeletedAndEdits() {
        Update update = mockCallbackUpdate(42L, 100L, 7, true, SubjectSettingsCallbackData.DEL_STATIC + "1");
    ...
    @Test
    void handle_withSystemPrompt_upsertsWithNullStatic() {
        var existing = new SubjectPromptDto(1L, "Math", "my prompt", null, "old static");
        Update update = mockCallbackUpdate(42L, 100L, 7, true, SubjectSettingsCallbackData.DEL_STATIC + "1");
        ...
        given(executorClient.upsertPrompt(eq(42L), eq(1L), any())).willReturn(
                new SubjectPromptDto(1L, "Math", "my prompt", null, null));
        ...
        assertThat(captor.getValue().systemPrompt()).isEqualTo("my prompt");
    }

    @Test
    void handle_withStaticOnly_deletesRecord() {
        var existing = new SubjectPromptDto(1L, "Math", null, null, "static text");
        Update update = mockCallbackUpdate(42L, 100L, 7, true, SubjectSettingsCallbackData.DEL_STATIC + "1");
