...
class SubjectSettingsPromptInputCallbackTest {
    ...
    SubjectSettingsPromptInputCallback callback;

    @BeforeEach
    void setUp() {
        callback = new SubjectSettingsPromptInputCallback(sender, autoHomeworkClient, stateService);
    }

    @Test
    void supports_returnsTrue_forSetPromptPrefix() {
        assertThat(callback.supports(SubjectSettingsCallbackData.SET_PROMPT + "1")).isTrue();
    }
    ...
    @Test
    void handle_accessibleMessage_startsStateAndShowsInputPrompt() {
        var specs = List.of(new JournalSpecResponse(1L, "Math", "MTH"));
        Update update = mockCallbackUpdate(42L, 100L, 7, true, SubjectSettingsCallbackData.SET_PROMPT + "1");
        ...
        verify(stateService).startInput(42L, 1L, "Math", 100L, 7,
                SubjectSettingsStateService.InputType.SYSTEM_PROMPT);
        verify(sender).answerCallback("cb-id");
        verify(sender).editHtml(eq(100L), eq(7), contains("AI промпт"), any());
    }

    @Test
    void handle_unknownSpec_usesIdFallbackName() {
        Update update = mockCallbackUpdate(42L, 100L, 7, true, SubjectSettingsCallbackData.SET_PROMPT + "99");
    ...
    void handle_inaccessibleMessage_answersWithError() {
        Update update = mockCallbackUpdate(42L, 0L, 0, false, SubjectSettingsCallbackData.SET_PROMPT + "1");
